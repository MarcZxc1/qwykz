...
export function generateContextPack(input: ProjectOptions | ScaffoldPlan): string {
  ...
  const stackSummary = [
    ...
    ...(options.learn ? [`- **Mode**      : hands-on learning (--learn active: follow GUIDE.md)`] : []),
    ...(ctx.orm ? [`- **ORM**       : ${ctx.orm}`] : []),
...
1. **Locate and inspect**: Read existing models, routes, and tests before introducing new libraries or files.
2. **Verification loop**: Run \`${ctx.packageManager === "cargo" ? "cargo test" : ctx.packageManager === "go" ? "go test ./..." : "bun test"}\` before and after modifications.
3. **Database consistency**: Run migrations immediately after modifying the database schema.
${options.learn ? "4. **Hands-On Mode**: This project is in learning mode. Guide the developer through GUIDE.md milestones rather than writing the complete application for them at once.\n" : ""}
---
