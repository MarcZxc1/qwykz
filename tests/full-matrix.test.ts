...
describe("Python (FastAPI)", () => {
  test("scaffold", () => {
    ...
    expect(fileExists(name, ".dockerignore")).toBe(true);
    expect(fileExists(name, ".env")).toBe(true);
    expect(fileContains(name, "Dockerfile", "COPY --chown=10001:10001 app ./app")).toBe(true);
    expect(fileContains(name, "requirements.txt", "fastapi[standard]")).toBe(false);
    expect(fileContains(name, "Dockerfile", '"app.main:app"')).toBe(true);
    expect(fileContains(name, "Dockerfile", "USER 10001:10001")).toBe(true);
    expect(fileContains(name, ".dockerignore", ".env*")).toBe(true);
...
describe("Go (Fiber)", () => {
  test("scaffold", () => {
    ...
    expect(fileExists(name, ".dockerignore")).toBe(true);
    expect(fileExists(name, ".env")).toBe(true);
    expect(fileContains(name, "Dockerfile", "./cmd/api")).toBe(true);
    expect(fileContains(name, "Dockerfile", "go mod tidy &&")).toBe(true);
    expect(fileContains(name, "Dockerfile", "CGO_ENABLED=0")).toBe(true);
    expect(fileContains(name, "Dockerfile", 'USER 10001:10001')).toBe(true);
    expect(fileContains(name, ".dockerignore", ".env*")).toBe(true);
...
describe("Rust (Axum)", () => {
  test("scaffold", () => {
    ...
    expect(fileExists(name, ".dockerignore")).toBe(true);
    expect(fileExists(name, ".env")).toBe(true);
    expect(fileContains(name, "Dockerfile", "FROM rust:1.88-slim-bookworm AS builder")).toBe(true);
    expect(fileContains(name, "Cargo.toml", '"runtime-tokio-rustls"')).toBe(true);
    expect(fileContains(name, "Cargo.toml", '"tls-native-tls"')).toBe(false);
    expect(fileContains(name, "Dockerfile", "printf 'fn main() {}\\n' > src/main.rs")).toBe(true);
    expect(fileContains(name, "Dockerfile", "target=/usr/src/app/target")).toBe(true);
    expect(fileContains(name, "Dockerfile", "USER 10001:10001")).toBe(true);
    expect(fileContains(name, ".dockerignore", "target/")).toBe(true);
    expect(fileContains(name, ".dockerignore", ".env*")).toBe(true);
